...
fn get_shortcuts_from_settings() -> (Shortcut, Shortcut) {
    ...
    (toggle_shortcut, hide_shortcut)
}

fn register_shortcuts(app: &AppHandle, toggle_shortcut: Shortcut, hide_shortcut: Shortcut) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn setup_shortcuts(app: &App) -> Result<(), Box<dyn std::error::Error>> {
    let (toggle_shortcut, hide_shortcut) = get_shortcuts_from_settings();
        ...
        tauri_plugin_global_shortcut::Builder::new()
            .with_handler(move |_app, _shortcut, event| {
                if event.state() == ShortcutState::Pressed {
                    let (current_toggle, current_hide) = get_shortcuts_from_settings();
    ...
    register_shortcuts(&app.handle(), toggle_shortcut, hide_shortcut)?;

    app.handle().listen("settings-changed", move |_| {
        let (new_toggle, new_hide) = get_shortcuts_from_settings();
        let _ = register_shortcuts(&app_handle, new_toggle, new_hide);
    });
